...
export function ReviewPage({ onNavigate, settings }: ReviewPageProps) {
  ...
  if (review.phase === 'idle') {
        ...
        <div className="flex min-h-[68vh] items-center justify-center animate-fade-in-up">
          <Card className="w-full max-w-2xl text-center">
            <CardContent className="flex flex-col items-center gap-6 px-6 py-14 sm:px-10">
              <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-[rgba(251,191,36,0.1)]">
                <PartyPopper className="h-11 w-11 text-[#fbbf24] animate-celebrate" />
              </div>
              <div className="space-y-2">
                <h2 className="text-[28px] font-bold tracking-[-0.025em]">{t('review.allCaughtUp', lang)}</h2>
    ...
    /* ── Idle: has due questions — start screen ── */
    return (
      <div className="animate-fade-in-up space-y-8">
        <div>
          <h1 className="text-[28px] font-bold tracking-[-0.025em]">{t('review.title', lang)}</h1>
          <p className="mt-1 text-base text-muted-foreground">{t('review.subtitle', lang)}</p>
        </div>
                ...
                <div className="rounded-xl border border-border bg-[rgba(255,255,255,0.03)] p-4">
                  ...
                  <div className="mt-3 space-y-2">
                    {Array.from(dueByTopic.entries()).slice(0, 4).map(([topicId, count]) => {
                      ...
                      return (
                        <div key={topicId} className="flex items-center justify-between gap-3 rounded-lg border border-border bg-[rgba(255,255,255,0.03)] px-3 py-2.5 text-sm">
      ...
      <div className="sticky top-0 z-20 mb-6 rounded-2xl border border-border bg-[rgba(10,10,15,0.85)] px-4 py-4 shadow-[0_0_0_1px_rgba(255,255,255,0.06),0_8px_24px_rgba(0,0,0,0.4)] backdrop-blur-xl">
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto text-tertiary">
